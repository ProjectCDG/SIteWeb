...
try {
	// Register the template in the database...
		  include('inc/db.php');
			...
			// Make the query:                                               #8
		$query = "UPDATE doc SET titre= $titre, description= $description, mention= $mention, conditionGene= $conditionGene WHERE id =" .$_GET['id'];	               // #9
        $q = mysqli_stmt_init($db);                                 // #10
        mysqli_stmt_prepare($q, $query);
     // execute query
        mysqli_stmt_execute($q);
        if (mysqli_stmt_affected_rows($q) == 1) {	// One record inserted	#11
		header ("location: confirm-modiftemplate.php");
		exit();
