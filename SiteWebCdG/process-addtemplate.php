
<?php
// This script is a query that INSERTs a record in the doc table.
// Check that form has been submitted:
	$errors = array(); // Initialize an error array. #2
	// Check titre:                        #3
		$titre = trim($_POST['titre']);
	if (empty($titre)) {
		$errors[] = 'titre manquant.';
	}
	// Check description:
		$description = trim($_POST['description']);
	if (empty($description)) {
		$errors[] = 'Description manquante.';
	}
	// Check mention:
		$mention = trim($_POST['mention']);
	if (empty($mention)) {
		$errors[] = 'Mention manquante.';
	}
  //check conditionGene
  $conditionGene = trim($_POST['conditionGene']);
if (empty($conditionGene)) {
  $errors[] = 'Condition Générale manquante.';
}

	if (empty($errors)) { // If everything's OK.              #5
try {
	// Register the template in the database...
		  include('inc/db.php');

			// Make the query:                                               #8
		$query = "INSERT INTO doc ( titre, description, mention, conditionGene) ";
		$query .="VALUES( ?, ?, ?, ? )";		               // #9
        $q = mysqli_stmt_init($db);                                 // #10
        mysqli_stmt_prepare($q, $query);
        // use prepared statement to insure that only text is inserted
        // bind fields to SQL Statement
        mysqli_stmt_bind_param($q, 'ssss', $titre, $description, $mention, $conditionGene);
     // execute query
        mysqli_stmt_execute($q);
        if (mysqli_stmt_affected_rows($q) == 1) {	// One record inserted	#11
		header ("location: addtemplateOK.php");
		exit();
		} else { // If it did not run OK.
		// Public message:
		    $errorstring = "<p class='text-center col-sm-8' style='color:red'>";
			$errorstring .= "System Error<br />You could not be registered due ";
			$errorstring .= "to a system error. We apologize for any inconvenience.</p>";
			echo "<p class=' text-center col-sm-2' style='color:red'>$errorstring</p>";
			// Debugging message below do not use in production
			echo '<p>' . mysqli_error($db) . '<br><br>Query: ' . $query . '</p>';
		    mysqli_close($db); // Close the database connection.
		// include footer then close program to stop execution
			echo '<footer class="jumbotron text-center col-sm-12"
	        style="padding-bottom:1px; padding-top:8px;">';
            include("footer.php");
            echo '</footer>';
		exit();
		}
	}
   catch(Exception $e) // We finally handle any problems here                #12
   {
     // print "An Exception occurred. Message: " . $e->getMessage();
     print "The system is busy please try later";
   }
   catch(Error $e)
   {
      //print "An Error occurred. Message: " . $e->getMessage();
      print "The system is busy please try again later.";
   }
	} else { // Report the errors.                                            #13
		$errorstring = "Error! <br /> The following error(s) occurred:<br>";
		foreach ($errors as $msg) { // Print each error.
			$errorstring .= " - $msg<br>\n";
		}
		$errorstring .= "Please try again.<br>";
		echo "<p class=' text-center col-sm-2' style='color:red'>$errorstring</p>";
		}// End of if (empty($errors)) IF.
?>
